    ...
    <form id="student-form" method="POST">
      <input type="hidden" name="id" value="<?= $id ?? "" ?>">
        <div class="row">
            <div class="col-12">
                <div class="mb-3">
                    <label for="class_id" class="form-label">Nom de la classe & Sujet</label>
                    <select type="text" class="form-select" id="class_id" name="class_id" required="required">
                      <option value="" <?= !isset($id) ? "selected" : "" ?> disabled> -- Sélectionnez la classe ici -- </option>
...
<script>
  $('#student-form').submit(function(e){
    ...
    $.ajax({
      ...
      dataType:'JSON',
      error: (err)=>{
        flashData.find('.flashdata-msg').text(`Une erreur s'est produite!`)
